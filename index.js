...
function iPickByRule(_, simply, rule, picker, type, filter) {
  var source = iMultiOrReKeys(
    rule,
    function (v, i, name, data) {
      var iChange,
        isAllPick = name.match(/(?:(?:(.*).|)\*$|\*\.)/);
      // 处理 data.*=>own.* 其中*是被复制
      isAllPick &&
        (iChange = isAllPick[1]) &&
        (data = iGetDataByLine(iChange, data, {}));
      ...
      // 按照顺序获取直到找到存在的对应key
      if (!isNil(i)) {
        var fi = iRuner(iPickNumber[(type && type[name]) || type], Nil, i),
          rData,
          iFiDate;
        i = isNumber(fi) ? fi : isNil(fi) ? i : Nil;
        iFiDate = isNil((rData = iRuner(filter, data, v, i, +this, name)));
...
function iPicker(source, rule, type, filter) {
  ...
  var rainbox = [],
    isArr = !(type === true || filter === true) && isArray(source),
    data;
  isArr || (source = [source]);
  // 遍历队列
